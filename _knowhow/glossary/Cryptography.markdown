...
Until the twentieth century, the use of cryptography was relatively minor and the methods used were often rudimentary.
...
### Cryptography
This is the science of the mathematical principles and methods applied for purposes such as data confidentiality or integrity. It tends to develop techniques for the storage of sensitive information and its transmission via insecure networks (such as the Internet) so that data can only be read or altered by authorised individuals.

### Cryptanalysis
Cryptanalysis is the study of the security of cryptographic mechanisms. It involves testing existing cryptographic functions to assess their strengths and weaknesses. Cryptanalysis is, therefore, a combination of analytical reasoning, the application of mathematical tools, the discovery of redundancies, patience, determination – and also luck.
...
### Key
The key is the value used in a cryptographic algorithm to encrypt a text. This is often a very large number, which size is measured in bits. The larger the key, the greater the security of the solution. However, it is the combination of algorithms and keys that guarantees the security of the solution. Keys must be stored securely so that only the owner can access and use them.

### Cryptosystem
A cryptosystem is an overall cryptographic solution. Such a system includes the encryption algorithm plus all the keys used and the protocols necessary for its operation.
...
### Examples
In English, the terms are as follows: single-key, one-key, private-key, symmetric-key encryption.
...
Public key cryptography is a response to the problem of key transmission in private key encryption. The concept was devised by Whitfield Diffie and Martin Hellman in 1975, although it is commonly accepted that the British secret service had already made the same discovery several years earlier, but it was protected as a military secret.
...
Note that the private key can also be used for encryption; it is then the public key that is used for the decryption. This property is used not for confidentiality but electronic signature (integrity).
