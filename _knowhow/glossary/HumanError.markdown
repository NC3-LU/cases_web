...
* not following procedures set out for saving data (SMEs: see Unusable backups),
* deactivating the antivirus update when starting up the computer,
* sharing their password with a colleague (SMEs: see Third-party use of access reserved for a single user),
* using the company’s IT architecture for personal use (SMEs: see Misuse of organisation’s resources)
* installing “non-standard” software on a machine, notably a computer or server (SMEs: see Use of unapproved software and Malicious administrator).
