...
## Fire
A fire can be a destructive [threat]({% link _knowhow/glossary/Threat.markdown %}), both to physical media (paper files) and to data stored on electronic devices (loss of [availability]({% link _knowhow/glossary/Availability.markdown %})). The source of this threat may be internal or external, deliberate or accidental. To prevent it, the following instructions should be followed:
...
* communication must be encrypted using a secure protocol (e.g. WPA2 for WiFi) (Draft and enforce a Sectoral policy on systems development and maintenance – [Use of encryption]({% link _knowhow/cisoapproach/SecurityPolicy-SystemDevelopmentAndMaintenance.markdown %}#encryption))
