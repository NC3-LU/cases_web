...
### 7Z
[7Z](https://www.7-zip.org/) is a free tool used to compress and archive files. This tool also has an AES 256-bit strong encryption option. It can, therefore, be used to transfer confidential encrypted files on removable devices or by email since the files are located in a compressed archive.
...
No key can be removed from the deposit. In case of usage, the follwing should be authorized:
...
Electronic signatures can be affixed to documents and emails. The signature guarantees the authenticity of the sender, as well as the [integrity]({% link _knowhow/glossary/Integrity.markdown %}) of the content of the file relating to the message.
